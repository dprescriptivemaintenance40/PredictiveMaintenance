...
export class CBAComponent implements OnInit {
  ...
  constructor(private rcmContantAPI: RCMContantAPI,
  ...
  async getPrescriptiveRecords() {
    ...
    var url: string = this.rcmContantAPI.PrescriptiveRecordsForCBA
  ...
  public cbasheet() {
    ...
    this.Economics = true;
    this.createColumns();
    this.SelectedPrescriptiveTree[0].failureModes.forEach(failuremodes => {
      failuremodes.MSS.forEach(MSSData => {
        this.MSSIndex++;
      });
  ...
  public SetCBAData() {
    ...
    this.FCAPattern = this.SelectedPrescriptiveTree[0].failureModes[0].Pattern;
    ...
    this.FCAConsequence = this.SelectedPrescriptiveTree[0].failureModes[0].Consequence.split(" ", 1);
    ...
    this.SelectedPrescriptiveTree[0].failureModes.forEach(failuremodes => {
      this.SetFailureMode = failuremodes.FailureMode;
  ...
  public SaveCBA() {
    if (this.RiskMatrix != '' && this.ScenarioYN != '') {
      ...
      CbaModelObj.PCMId = 0;
      CbaModelObj.RCMId = this.SelectedPrescriptiveTree[0].RCMId;
      CbaModelObj.FailureModeId = this.SelectedPrescriptiveTree[0].failureModes[0].FailureModeId;
      ...
      var url: string = this.rcmContantAPI.CBASheet
      this.prescriptiveBLService.postWithoutHeaders(url, CbaModelObj)
        .subscribe(
          res => {
            console.log(res);
            this.messageService.add({ severity: 'success', summary: 'success', detail: 'Successfully Updated CBA' });
