...
export class FCAComponent implements OnInit {
  ...
  SaveFCA() {
    ...
    for (let index = 0; index < this.data1[0].children[0].children[0].children.length; index++) {
      ...
      RCMOBJ.failureModes.push(obj)
    }
    var url: string = this.RCMContantAPI.FCASave
    this.RCMBLService.PutData(url, RCMOBJ).subscribe(
      res => {
